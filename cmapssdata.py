...
class CMAPSSDataset():
    def __init__(self, fd_number, batch_size, sequence_length):
        ...
        norm_data= pd.DataFrame(self.min_max.fit_transform(data[cols_normalize]),
                                 columns=cols_normalize, index=data.index)
        ...
        #rul_max 两部分组成，一部分是RUL文件中的已有的elaspsed，一部分是计算到结束的
        ...
        norm_test_data = pd.DataFrame(self.min_max.fit_transform(test_data[cols_normalize]),
